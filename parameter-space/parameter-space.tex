...
\begin{equation}
  \frac{da}{dt} = \frac{\xi_\text{a} \rho\rms{C} w_\text{a}}{4 \rho\rms{gr}} ,
\end{equation}

\noindent
where $w_a$ is the Maxwell-Boltzmann distribution RMS velocity, $\xi_a$ is the grain sticking efficiency, $\rho\rms{C}$ is the carbon density in the gas and $\rho_\text{gr}$ is the grain bulk density.
The associated rate of dust density change is found to be

\begin{equation}
  \frac{d\rho_\text{d}}{dt}  = 4 \pi a^2 \rho_\text{C} n_\text{d} \frac{da}{dt} , 
\end{equation}

\noindent
where $n_\text{d}$ is the grain number density.
...
Dust destruction is calculated via gas-grain sputtering using the \cite{draine_destruction_1979} prescription - a dust grain has a normalised lifespan, $\tau$, which is dependent on the grain radius, and the number density of the gas the grain is moving through, $n\rms{g}$.
In the case of spherical amorphous carbon grains, this normalised lifespan is found to be $\tau\rms{d} \approx \SI{1}{\mega\year} \times a/n\rms{g}$.
...
\begin{subequations}
  \begin{align}
    \frac{da}{dt} & = - \frac{a}{\tau \rms d} , \\
    \frac{dm\rms{gr}}{dt} & = \frac{m \rms d}{\tau\rms{d}} = \frac{4 \pi a^3 \rho\rms{gr} }{3 \tau\rms{d}} , \\
    \frac{d \rho\rms{d}}{dt} & = \frac{dm}{dt} n\rms{d} .
  \end{align}
\end{subequations}
