...
\begin{subequations}
  \begin{align}
    ...
    \frac{\partial \rho \varepsilon}{\partial t} & + \nabla \cdot \left[ \boldsymbol{u} \left( \rho\varepsilon + P \right) \right] = \frac{d E\rms{cool}}{dt} , 
  \end{align}
\end{subequations}

\noindent
where $\varepsilon$ is the total specific energy ($\varepsilon = \boldsymbol{u}^2/2 + e/\rho $), $\rho$ is the gas density, $e$ is the internal energy density, $P$ is the gas pressure, $\boldsymbol{u}$ is the gas velocity and $dE\rms{cool}/dt$ is the energy loss rate per unit volume from the fluid due to gas and dust cooling.
...
In this paper we take $\xi = 0.1$ as a conservative value, though this value can rise to as high as $1$ in the case of highly charged grains \citep{spitzer_jr._physical_2008}.
...
\begin{subequations}
  \begin{align}
    ...
    \frac{d m\rms{gr}}{dt}   & = -4 \pi \frac{\rho\rms{gr} a^3}{\tau\rms{d}} = -4\pi \frac{\rho\rms{gr} n\rms{g} a^2}{\num{3e6}} ,\\
...
We also observe that simulations with wind have a markedly more mixed wind, due to instabilities in the post-shock environment (Fig. \ref{fig:radiative-windmixing}).
...
Through these simulations we have determined a series of tendencies regarding the change in dust yield for systems with differing wind and orbital parameters.
...
As the wind mass loss rates change, we find that dust production occurs if either star has a greater mass loss rate.
The system appears to undergo more cooling.
This change could be attributed to the increased number density of dust grains in the WC wind in the case of simulation \texttt{mdot-1}.
...
If the wind \emph{velocities} are more imbalanced we find that dust production rate increases significantly as well.
This is most likely due to the presence of KH instabilities from an increased wind shear.
Dust production is also affected by orbital properties, as the separation distance increases, we find a corresponding drop in 
due to the reduction in instabilities generating high-density clumps, as well as the reduced shock intensity as the winds become more diffuse with distance.
...
The production of dust in high density clumped regions could also explain the formation of dust in single WR/OB systems, which can form dust in significantly lower quantities.

Furthermore, we note that the dust production rate of the \texttt{baseline} simulation is somewhat lower than the predicted dust production rate of WR98a.
...
\noindent
Whilst this is the case, a qualitative analysis of which wind and orbital parameters significantly impact formation rates has been performed.
A more in-depth analysis of WCd systems using an improved version of this model would allow us to make quantitative analyses of dust formation rate.
